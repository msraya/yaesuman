...
CData::CData(char *callsign, char *Gps)
{
    m_head = NULL;
    m_tail = NULL;
    m_actual = NULL;
...
void CData::config(char *callsign, char *Gps){
    strcpyfill((uint8_t *)m_config.callsign,callsign,16U);
    if (strlen(Gps) == 20U) strcpy(m_config.Gps,Gps);
    else memset(m_config.Gps,0x00,20U);
}
...
//	MessageBox(0, m_basename, "Oh Oh...", MB_ICONEXCLAMATION | MB_OK);
...
void CData::add_pnode(uint8_t *data){
    ...
    if (data == NULL) {
	    ...
	    tmp_cad[6U]=0;
	    if (strlen(tmp_cad) == 6U) strcpy(m_signature,tmp_cad);
